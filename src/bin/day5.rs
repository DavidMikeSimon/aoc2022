use std::{error, fs, path, io::{self, BufRead}, iter, collections::VecDeque};
...
fn main() -> Result<(), Box<dyn error::Error>> {
    ...
    let mut columns: Vec<VecDeque<char>> =
        iter::repeat_with(|| VecDeque::<char>::new()).take(num_columns).collect(); 
    ...
    for line in lines {
        ...
        if let Some(cap) = maybe_cap {
            ...
            for _ in 0..quantity {
                let item = columns.get_mut(src-1).ok_or("Bad source column index")?.pop_back().ok_or("Empty source column")?;
                // columns.get_mut(dst-1).ok_or("Bad dest column")?.push_back(item);
                stack.push_front(item);
            }
            columns.get_mut(dst-1).ok_or("Bad dest column")?.append(&mut stack);
        }
    }
